...
public class MEL {
	...
	class DrawNoteSet extends PApplet {
		...
		private void drawNote(Note n) {
			System.out.println(n+" priority:"+n.priority+" maxPriority:"+maxPriority);
			float col = map(n.priority, 0, maxPriority, 0, 150);
			stroke(col);
			fill(0);
			strokeWeight(2);
			strokeCap(ROUND);
			...
			float d = map(n.priority, 0,maxPriority, 16,8);
			ellipse(x,y,d,d);
		}
	...
	private static final String THIS_LANGUAGE_IDENTIFIER = "MEL23";

	private static String HOME_DIR = "data/"+THIS_LANGUAGE_IDENTIFIER.toLowerCase();
	...
	protected Vector vector(Coords coords) {
		return new Vector(coords.time,coords.pitch,noteSpaceTimeMaskSequence,noteSpacePitchMaskSequence);
	}
