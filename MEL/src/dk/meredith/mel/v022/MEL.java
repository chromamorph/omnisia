...
public class MEL {
	...
	class DrawNoteSet extends PApplet {
		...
		private void drawNote(Note n) {
			...
			float d = 1+3*(maxPriority-n.priority);
			ellipse(x,y,d,d);
		}
	...
	class Seq {
		private ArrayList<Object> seq = new ArrayList<Object>();
	...
	private static final String THIS_LANGUAGE_IDENTIFIER = "MEL22";
	...
	private ArrayList<NotePair> DRAWN_VECTORS = new ArrayList<NotePair>();
	...
	protected VectorSumSet product(Object... X) {
		...
		for(int i = 0; i < X.length; i++) {
			if (X[i] instanceof VectorCollection)
				Y[i] = vectorSumSet((VectorCollection)X[i]);
			else if (X[i] instanceof VectorOrVectorSum)
				Y[i] = vectorSumSet((VectorOrVectorSum)X[i]);
			else if (X[i] instanceof Seq) {
				Seq xSeq = (Seq)(X[i]);
				ArrayList<VectorSumSet> ySeq = new ArrayList<VectorSumSet>();
				for(int j = 0; j < xSeq.size(); j++) {
					if(xSeq.get(j) instanceof VectorCollection)
						ySeq.add(vectorSumSet((VectorCollection)(xSeq.get(j))));
					else if (xSeq.get(j) instanceof VectorOrVectorSum)
						ySeq.add(vectorSumSet((VectorOrVectorSum)(xSeq.get(j))));
				}
				Y[i] = ySeq;
			}
		}
		...
		for(int i = 0; i < Y.length; i++) {
			if (Y[i] instanceof VectorSumSet)
				((VectorSumSet)Y[i]).add(vectorSum(zeroVector));
			else {
				@SuppressWarnings("unchecked")
				ArrayList<VectorSumSet> sumSetList = (ArrayList<VectorSumSet>)(Y[i]);
				for(VectorSumSet s : sumSetList)
					s.add(vectorSum(zeroVector));
				System.out.println("{");
				for(VectorSumSet v : sumSetList)
					System.out.println(v);
				System.out.println("}");
			}
		}
		
		VectorSumSet vectorSumSet1 = copy((VectorSumSet)Y[0]);
		for(int i = 1; i < Y.length; i++) {
			VectorSumSet vectorSumSet2 = vectorSumSet();
			for (int k = 0; k < vectorSumSet1.size(); k++) {
				if (Y[i] instanceof VectorSumSet) {
					for(int j = 0; j < ((VectorSumSet)(Y[i])).size(); j++)
						vectorSumSet2.add(vectorSum(vectorSumSet1.get(k),((VectorSumSet)(Y[i])).get(j)));
				} else {
					@SuppressWarnings("unchecked")
					ArrayList<VectorSumSet> sumSetList = (ArrayList<VectorSumSet>)(Y[i]);
					for(int j = 0; j < sumSetList.get(k%sumSetList.size()).size(); j++)
						vectorSumSet2.add(vectorSum(vectorSumSet1.get(k),sumSetList.get(k%sumSetList.size()).get(j)));
				}
			}
	...
	protected Seq sequence(Object... objects) {
		return new Seq(objects);
	}
