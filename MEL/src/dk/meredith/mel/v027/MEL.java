...
//			float col = map(n.priority, 0, max(maxPriority,1), 0, lightestNoteShade);
			stroke(0);
			fill(0);
			strokeWeight(2);
			strokeCap(ROUND);
			float x = map(time(n),minTime,max(1,maxTime),minX,maxX);
			float y = map(pitch(n),minPitch,max(1,maxPitch),maxY,minY);
			float d = noteSize;//map(n.priority, 0,max(1,maxPriority), largestNoteSize,smallestNoteSize);
			ellipse(x,y,d,d);
		}
	...
	class MaskSequence implements Comparable<MaskSequence>, MaskOrMaskSequence {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("maskSequence(");
			if (masks.size() == 0)
				sb.append(")");
			else {
				sb.append(masks.get(0).toString());
				for(int i = 1; i < masks.size();i++)
					sb.append(","+masks.get(i));
				sb.append(")");
			}
			return sb.toString();
		}
	...
	class MaskStructure implements Comparable<MaskStructure> {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("maskStructure("+intervals.get(0));
...
//			if (d != 0) return d;
		...
		public String toString() {
			return "note("+time+","+pitch+")";
		}

		public Note translate(Vector vector) {
			return new Note(time+vector.time,pitch+vector.pitch);
		}

		public Note copy() {
			Note n = new Note(time, pitch);//, priority);
			return n;
		}
...
//		}
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("noteSet(");
			if (size() == 0)
				sb.append(")");
			else {
				ArrayList<Note> array = new ArrayList<Note>(notes);
		...
		public NoteSet copy() {
			NoteSet newNoteSet = new NoteSet();
			@SuppressWarnings("unchecked")
			TreeSet<Note> newNoteTreeSet = (TreeSet<Note>)notes.clone();
			newNoteSet.notes = newNoteTreeSet;
			return newNoteSet;
		}
		...
		public void remove(NoteSet noteSet) {
			for(Note note : noteSet.getNotes())
				remove(note);
		}

		public NoteSet translate(Vector vector) {
			NoteSet newNoteSet = new NoteSet();
			for(Note note : notes)
				newNoteSet.add(new Note(note.time+vector.time,note.pitch+vector.pitch));
			return newNoteSet;
		}

		public boolean translationallyEquivalentTo(NoteSet noteSet) {
			if (noteSet == null) return false;
			if (noteSet.size() != size()) return false;
			Vector vector = null;
			for(int i = 0; i < size(); i++) {
				if (vector == null)
					vector = new Vector(get(i),noteSet.get(i));
				else if (!vector.equals(new Vector(get(i),noteSet.get(i))))
					return false;
			}
			return true;
		}
		...
		public int compareTo(NoteSet noteSet) {
			if (noteSet == null) return 1;
			int d = size() - noteSet.size();
			if (d != 0) return d;
			//Same size
			ArrayList<Note> a1 = new ArrayList<Note>(notes);
			ArrayList<Note> a2 = new ArrayList<Note>(noteSet.getNotes());
		...
		public boolean equals(Object obj) {
			if (obj == null) return false;
			if (!(obj instanceof NoteSet)) return false;
			return compareTo((NoteSet)obj) == 0;
		}
	...
	class Space {
		...
		public String toString() {
			return "space("+timeMaskSequence+","+pitchMaskSequence+")";
		}
	}
	...
	class Vector implements VectorOrVectorSum, Comparable<Vector> {
		...
		public Vector(Note n1, Note n2) {
			this.time = n2.time-n1.time;
			this.pitch = n2.pitch-n1.pitch;
		}
		...
		public String toString() {
			if (timeMaskSequence.equals(maskSequence(mask(0,1))) &&
					pitchMaskSequence.equals(maskSequence(mask(0,1)))) {
				return "vector("+time+","+pitch+")";
			}
			return "vector("+time+","+pitch+","+timeMaskSequence+","+pitchMaskSequence+")";
		}
	...
	class VectorSequence implements VectorSequenceOrVectorSumSequence, VectorCollection, Comparable<VectorSequence> {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("vectorSequence(");
			if (vectors.size() == 0)
				return sb.append(")").toString();
			else 
				sb.append(vectors.get(0));
			for(int k = 1; k < vectors.size(); k++)
				sb.append(","+vectors.get(k));
			return sb.append(")").toString();
		}

		@Override
		public int compareTo(VectorSequence q) {
			if (q == null) return 1;
			int d = length() - q.length();
			if (d != 0) return d;
			for (int i = 0; i < length(); i++) {
				d = get(i).compareTo(q.get(i));
				if (d != 0) return d;
			}
			return 0;
		}
		...
		public boolean equals(Object o) {
			...
			if (!(o instanceof VectorSequence)) return false;
			return compareTo((VectorSequence)o) == 0;
		}
	}
	...
	class VectorSet implements VectorCollection, Comparable<VectorSet> {
		...
		VectorSet(TreeSet<Vector> vectors) {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("vectorSet(");
			if (vectors.size() > 0) {
		...
		public TreeSet<Vector> getVectors() {
			return (TreeSet<Vector>)vectors.clone();
		}

		@SuppressWarnings("unchecked")
		public VectorSet copy() {
			return new VectorSet((TreeSet<Vector>)vectors.clone());
		}
		...
		public void remove(VectorSet vectorSet) {
			for(Vector vector : vectorSet.getVectors()) 
				remove(vector);
		}

		@Override
		public int compareTo(VectorSet s) {
			if (s == null) return 1;
			int d = size() - s.size();
			if (d != 0) return d;
			for(int i = 0; i < s.size(); i++) {
				d = get(i).compareTo(s.get(i));
				if (d != 0) return d;
			}
			return 0;
		}
		...
		public boolean equals(Object o) {
			...
			return compareTo((VectorSet)o) == 0;
		}
	}
	...
	class VectorSum implements VectorOrVectorSum, Comparable<VectorSum> {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("vectorSum(");
			if (vectors.size() != 0) {
				sb.append(vectors.get(0));
				for(int k = 1; k < vectors.size(); k++)
					sb.append(","+vectors.get(k));
	...
	class VectorSumSequence implements VectorSequenceOrVectorSumSequence, VectorCollection {
		...
		public VectorSumSet getVectorSumSet() {
//			System.out.println("About to get vector sum set from vector sum sequence: "+this);
			VectorSumSet s = new VectorSumSet();
//			System.out.println("vectorSums.size() =="+ vectorSums.size());
...
//			System.out.println("s = "+s);
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("vectorSumSequence(");
			if (vectorSums.size() != 0) {
				sb.append(vectorSums.get(0));
				for(int k = 1; k < vectorSums.size(); k++)
					sb.append(","+vectorSums.get(k));
			}
			return sb.append(")").toString();
		}
	}
	...
	class VectorSumSet implements VectorCollection {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder("vectorSumSet(");
			if (vectorSums.size() != 0) {
				sb.append(vectorSums.first());
				for(VectorSum vectorSum : vectorSums.tailSet(vectorSums.first(), false))
					sb.append(","+vectorSum);
			}
			return sb.append(")").toString();
		}
	...
	private static final String THIS_LANGUAGE_IDENTIFIER = "MEL27";
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				chooser.setCurrentDirectory(new File("/Users/dave/Documents/Work/Research/2015-06-17-workspace/MEL/data"));
	...
	protected NoteSet add(Note... notes) {
		this.notes.add(notes);
		NoteSet noteSet = new NoteSet(notes);
		return noteSet;
	}

	protected NoteSet add(NoteSet... noteSets) {
		NoteSet outputSet = new NoteSet();
		for(NoteSet noteSet : noteSets) {
			outputSet.add(noteSet);
			notes.add(noteSet);
		}
		return outputSet;
	}
...
//				System.out.println("About to convert a VectorCollection to a VectorSumSetSequence");
				Y[i] = new VectorSumSetSequence((VectorCollection)X[i]);
			}
			else
				Y[i] = new VectorSumSetSequence(new VectorSumSet((VectorOrVectorSum)X[i]));
		}
//		System.out.println("Y before adding zero vector:");
...
//		System.out.println("Y after adding zero vector:");
//		for(VectorSumSetSequence vsss : Y)
//			System.out.println(vsss);
...
//	}
	...
	class VectorSequenceNotePair implements Comparable<VectorSequenceNotePair> {
		private VectorSequence vectorSequence;
		private Note note;

		VectorSequenceNotePair(VectorSequence vectorSequence, Note note) {
			this.vectorSequence = vectorSequence;
			this.note = note;
		}

		VectorSequenceNotePair(NoteSet noteSet) {
			note = noteSet.first();
			vectorSequence = new VectorSequence();
			for(int i = 0; i < noteSet.size()-1; i++)
				vectorSequence.add(new Vector(noteSet.get(i),noteSet.get(i+1)));
		}

		@Override
		public int compareTo(VectorSequenceNotePair x) {
			if (x == null) return 1;
			return vectorSequence.compareTo(x.vectorSequence);
		}

		public boolean equals(Object o) {
			if (o == null) return false;
			if (!(o instanceof VectorSequenceNotePair)) return false;
			return compareTo((VectorSequenceNotePair)o) == 0;
		}

		public NoteSet getNoteSet() {
			NoteSet N = new NoteSet(note);
			Note n = note.copy();
			for(Vector v : vectorSequence.vectors) {
				n = n.translate(v);
				N.add(n);
			}
			return N;
		...
		public String toString() {
			return "vectorSequenceNotePair("+vectorSequence+","+note+")";
		}
	}

	/**
	 * Tries to compute the shortest MEL program that generates this note set.
	 */
	protected void encode() {
		NoteSet notesLeft = notes.copy();
		int sizeOfOutput = 0;
		StringBuilder sb = new StringBuilder(THIS_LANGUAGE_IDENTIFIER+";");
		while(!notesLeft.isEmpty()) {
			//Run SIATEC on notesLeft and save the TEC that has the best compression ratio
			//Uses algorithm described in Meredith et al 2002. p.335ff

			//SIATEC: Step 1 - Sorting the dataset
			//notesLeft is already sorted

			//SIATEC: Step 2 - Computing W
			int n = notesLeft.size();
			
			if (n == 1) {
				String s = "\nadd("+notesLeft+");// cr=1.0]";
				sb.append(s);
				notesLeft.remove(notesLeft);
				sizeOfOutput++;
				break;
			}
			
			VectorNotePair[][] W = new VectorNotePair[n][n];

			for(int i = 0; i < n; i++)
				for(int j = 0; j < n; j++)
					W[i][j] = new VectorNotePair(new Vector(notesLeft.get(i),notesLeft.get(j)),notesLeft.get(i));

			//SIATEC: Step 3 - Computing V
			//SIATEC: Step 4 - Sorting V to produce _V_
			//These two steps can be done in one go by storing the <vector,note> pairs in a TreeSet

			TreeSet<VectorNotePair> V = new TreeSet<VectorNotePair>();
			for(int i = 0; i < n-1; i++)
				for(int j = i+1; j < n; j++)
					V.add(W[i][j]);

			//SIATEC: Step 5 - Vectorizing the MTPs
			//SIATEC: Step 6 - Sorting X
			//These two steps find a set of translationally distinct MTPs

			TreeSet<VectorSequenceNotePair> X = new TreeSet<VectorSequenceNotePair>();
			Vector thisVector = V.first().vector;
			NoteSet thisMTP = new NoteSet();
			for(VectorNotePair vn : V) {
				if (vn.vector.equals(thisVector))
					thisMTP.add(vn.note);
				else {
					X.add(new VectorSequenceNotePair(thisMTP));
					thisVector = vn.vector;
					thisMTP = new NoteSet(vn.note);
				}
			}
			X.add(new VectorSequenceNotePair(thisMTP));
			
			//Now we turn the VectorSequenceNotePairs into NoteSets
			TreeSet<NoteSet> MTPs = new TreeSet<NoteSet>();
			for(VectorSequenceNotePair x : X)
				MTPs.add(x.getNoteSet());

			//Now for each MTP we need to find its TEC and add it to the encoding
			//if it gives the best compression ratio.

			TEC bestTEC = null;
			double bestCR = 0.0;

			for(NoteSet mtp : MTPs) {
				//Find translators
				//Based on PRINT_SET_OF_TRANSLATORS algorithm (Meredith et al., 2002, p.339)
				VectorSet translators = new VectorSet();

				//Find column indices into W for notes in mtp
				int m = mtp.size();
				int[] I = new int[m];
				int i = 0, l = 0;
				for(Note note : mtp.getNotes()) {
					while(i < notesLeft.size() && !notesLeft.get(i).equals(note))
						i++;
					if (i == notesLeft.size()) {
						System.out.println("ERROR! "+note+" is not in "+notesLeft);
						System.exit(1);
					}
					I[l] = i;
					l++;
				...
				//Now find translators
				if (m == 1) {
					for(int k = 0; k < n; k++)
						translators.add(W[I[0]][k].getVector());
				} else {
					int[] J = new int[m];
					for(int k = 0; k < m; k++) J[k] = 0;
					boolean finished = false;
					int k = 1;
					while(!finished) {
						if (J[k] < J[k-1])
							J[k] = J[k-1]+1;
						while(J[k] <= n - m + k && W[I[k]][J[k]].getVector().lessThan(W[I[k-1]][J[k-1]].getVector()))
							J[k]++;
						if (J[k] > n - m + k) finished = true;
						else if (W[I[k]][J[k]].getVector().greaterThan(W[I[k-1]][J[k-1]].getVector())) {
							J[0]++;
							k = 1;
							if (J[0] > n - m) finished = true;
						} else if (k == m - 1) { //Vectors selected in all columns are equal
							translators.add(W[I[k]][J[k]].getVector());
							for(int j = 0; j < m; j++) {
								J[j]++;
								if (J[j] > n - m + j) {
									finished = true;
				...
				//Make TEC for this MTP
				TEC tec = new TEC(mtp,translators);
//				System.out.println("TEC before reduction: "+tec);
				tec.reduceTranslators();

				//Find out if it's the best TEC so far
				double cr = tec.getCompressionRatio();
//				System.out.println("TEC: "+tec+" cr="+String.format("%.2f",cr));
				if (bestTEC == null || cr > bestCR) {
					bestTEC = tec.copy();
					bestCR = cr;
				}
			...
			String s = "\n"+bestTEC+";// cr="+String.format("%.2f",bestCR);
			sb.append(s);
			notesLeft.remove(bestTEC.getNotes());
			sizeOfOutput += bestTEC.getTranslators().size()+bestTEC.getPattern().size();
		}
		sb.append("\n//Overall compression ratio = "+String.format("%.2f",1.0*notes.size()/sizeOfOutput));
		sb.append("\ndraw();\nprint();");
		System.out.println("\n\n>>>START ENCODING<<<\n"+sb.toString()+"\n>>>END ENCODING<<<\n");
		//Write sb to new encoding file
		int n = 0;
		String encodedFileName = fileName.substring(0,fileName.length()-4)+"-enc"+String.format("%03d",n)+".mel";
		while (new File(encodedFileName).exists()) {
			encodedFileName = fileName.substring(0,fileName.length()-4)+"-enc"+String.format("%03d",++n)+".mel";
		}
		try {
			PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(encodedFileName)));
	...
	public TreeSet<NoteSet> SIA(NoteSet notes) {
		if (notes.size() == 1) {
			TreeSet<NoteSet> m = new TreeSet<NoteSet>();
			m.add(notes);
			return m;
		}
		TreeSet<VectorNotePair> vectorTable = new TreeSet<VectorNotePair>();
		TreeSet<NoteSet> mtps = new TreeSet<NoteSet>();
		ArrayList<Note> noteList = new ArrayList<Note>(notes.getNotes());
		for(int i = 0; i < noteList.size() - 1; i++)
			for(int j = i+1; j < noteList.size(); j++)
				vectorTable.add(new VectorNotePair(new Vector(noteList.get(i),noteList.get(j)), noteList.get(i)));
		Vector thisVector = vectorTable.first().getVector();
		NoteSet mtp = new NoteSet();
		for(VectorNotePair vectorNotePair : vectorTable) {
			if (vectorNotePair.getVector().equals(thisVector))
				mtp.add(vectorNotePair.getNote());
			else {
				boolean translationallyDistinct = true;
				for(NoteSet prevMTP : mtps) {
					if (mtp.translationallyEquivalentTo(prevMTP)) {
						translationallyDistinct = false;
						break;
					}
				}
				if (translationallyDistinct)
					mtps.add(mtp);
				mtp = new NoteSet();
				mtp.add(vectorNotePair.getNote());
				thisVector = vectorNotePair.getVector();
			}
		}
		boolean translationallyDistinct = true;
		for(NoteSet prevMTP : mtps) {
	...
	class VectorNotePair implements Comparable<VectorNotePair> {
		private Vector vector;
		private Note note;

		public VectorNotePair(Vector vector, Note note) {
			this.vector = vector;
			this.note = note;
		}
		...
		@Override
		public int compareTo(VectorNotePair vp) {
			if (vp == null) return 1;
			int d = vector.compareTo(vp.vector);
			if (d != 0) return d;
			d = note.compareTo(vp.note);
			if (d != 0) return d;
			return 0;
		}

		public boolean lessThan(VectorNotePair vp) {
			return compareTo(vp) < 0;
		}

		@Override
		public boolean equals(Object obj) {
			if (obj == null) return false;
			if (!(obj instanceof VectorNotePair)) return false;
			return compareTo((VectorNotePair)obj) == 0;
		}
	}


	public class TEC {
		private NoteSet pattern;
		private VectorSet translators;

		public TEC(NoteSet pattern, NoteSet dataset) {
			VectorSet vectors = new VectorSet();
			Note firstPatternNote = pattern.first();
			for(Note note : dataset.getNotes())
				vectors.add(new Vector(firstPatternNote,note));
			for(Note patternNote : pattern.getNotes().tailSet(firstPatternNote,false)) {
				VectorSet thisNotesVectors = new VectorSet();
				for(Note datasetPoint : dataset.getNotes())
					thisNotesVectors.add(new Vector(patternNote,datasetPoint));
				VectorSet newVectors = vectors.copy();
				for(Vector vector : vectors.getVectors())
					if(!thisNotesVectors.contains(vector))
						newVectors.remove(vector);
				vectors = newVectors;
			}
			this.pattern = pattern;
			vectors.remove(new Vector(0,0));
			this.translators = vectors;
			normalize();
		}

		public TEC(NoteSet pattern, VectorSet translators) {
			setPattern(pattern);
			translators.remove(new Vector(0,0));
			setTranslators(translators);
			normalize();
		}

		private void setPattern(NoteSet pattern) {
			this.pattern = pattern;
		}

		private void setTranslators(VectorSet translators) {
			this.translators = translators;
		}

		public NoteSet getNotes() {
			NoteSet newNotes = pattern.copy();
			for(Note note : pattern.getNotes())
				for(Vector vector : translators.getVectors())
					newNotes.add(note.translate(vector));
			return newNotes;
		}

		public double getCompressionRatio() {
			return (1.0 * getNotes().size())/(pattern.size()+translators.size());
		}

		public TEC copy() {
			return new TEC(pattern.copy(), translators.copy());
		}

		public String toString() {
			if (translators.isEmpty())
				return "add("+pattern.toString()+")";
			return "add(translate(add("+pattern+"),"+translators+"))";
		}

		public TEC reduceTranslators() {
			NoteSet thisCoveredSet = getNotes();
			ArrayList<VectorSet> removableSubsets = new ArrayList<VectorSet>();
			//First we try to remove just one vector from translators
			VectorSet newTranslators = translators.copy();
			for(Vector vectorToBeRemoved : translators.getVectors()) {
				newTranslators.remove(vectorToBeRemoved);
				NoteSet coveredSet = new TEC(pattern,newTranslators).getNotes();
				if (coveredSet.equals(thisCoveredSet))
					removableSubsets.add(new VectorSet(vectorToBeRemoved));
				newTranslators.add(vectorToBeRemoved);
			}
			//Now we try to add more translators to each of the removableSubsets
			boolean removableSubsetsChanged = (removableSubsets.size() > 0);
			while (removableSubsetsChanged) {
				removableSubsetsChanged = false;
				for(int i = 0; i < removableSubsets.size(); i++) {
					VectorSet removableSubset = removableSubsets.get(i);
					newTranslators = translators.copy();
					newTranslators.remove(removableSubset);
					@SuppressWarnings("unchecked")
					TreeSet<Vector> newTranslatorVectors = (TreeSet<Vector>)newTranslators.getVectors().clone();
					for(Vector nextVectorToBeRemoved : newTranslatorVectors) {
						newTranslators.remove(nextVectorToBeRemoved);
						NoteSet coveredSet = new TEC(pattern,newTranslators).getNotes();
						if (coveredSet.equals(thisCoveredSet)) {
							VectorSet newVectorSet = removableSubset.copy();
							newVectorSet.add(nextVectorToBeRemoved);
							removableSubsets.add(newVectorSet);
							removableSubsetsChanged = true;
						}
						newTranslators.add(nextVectorToBeRemoved);
					}
				}
				if (removableSubsetsChanged) {
					//Remove all but the longest removableSubsets
					//First find length of largest removable subset(s)
					int lengthOfLongestRemovableSubset = 0;
					for(int i = 0; i < removableSubsets.size(); i++)
						if (removableSubsets.get(i).size() > lengthOfLongestRemovableSubset)
							lengthOfLongestRemovableSubset = removableSubsets.get(i).size();
					//Now make a new list of removable subsets that contains only the longest ones
					ArrayList<VectorSet> newRemovableSubsets = new ArrayList<VectorSet>();			
					for(int i = 0; i < removableSubsets.size(); i++)
						if (removableSubsets.get(i).size() == lengthOfLongestRemovableSubset)
							newRemovableSubsets.add(removableSubsets.get(i));
					removableSubsets = newRemovableSubsets;
				}
			}
			if (removableSubsets.size() > 0) {
				//				System.out.print("TEC changed: "+this+" reduced to ");
				translators.remove(removableSubsets.get(0));
				//				System.out.println(this + " by removing "+removableSubsets.get(0) + ". translators = "+translators);
			}
			normalize();
			return this;
		}
		...
		/**
		 * Resets pattern and translators so that pattern is the least of all
		 * the patterns in the TEC.
		 */
		private void normalize() {
			TreeSet<NoteSet> patterns = new TreeSet<NoteSet>();
			patterns.add(pattern);
			for(Vector translator : translators.getVectors())
				patterns.add(pattern.translate(translator));
			NoteSet newPattern = patterns.first();
			VectorSet newTranslators = new VectorSet();
			for(NoteSet pat : patterns)
				newTranslators.add(new Vector(newPattern.first(),pat.first()));
			pattern = newPattern;
			newTranslators.remove(new Vector(0,0));
			translators = newTranslators;
		}

		public NoteSet getPattern() {
			return pattern;
		}
