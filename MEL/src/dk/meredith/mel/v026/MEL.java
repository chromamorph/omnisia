...
public class MEL {
	...
	class DrawNoteSet extends PApplet {
		...
		private void drawNote(Note n) {
			...
			float d = map(n.priority, 0,max(1,maxPriority), largestNoteSize,smallestNoteSize);
			System.out.println("Diameter of note is "+d);
			ellipse(x,y,d,d);
		}
	...
	class VectorSequence implements VectorSequenceOrVectorSumSequence, VectorCollection {
		...
		public VectorSumSet getVectorSumSet() {
			System.out.println("About to get VectorSumSet from VectorSequence: "+this);
	...
	private static final String THIS_LANGUAGE_IDENTIFIER = "MEL26";
	...
	protected VectorSumSet product(Object... X) {
		...
		for(int i = 0; i < X.length; i++) {
			if (X[i] instanceof VectorSumSetSequence)
				...
				System.out.println("About to turn a VectorCollection into a VectorSumSetSequence");
	...
	protected void encode() {
		...
		while(!notesLeft.isEmpty()) {
			...
			TEC bestTEC = null;
			TreeSet<NoteSet> MTPs = SIA(notesLeft);
			for(NoteSet mtp : MTPs) {
				TEC tec = new TEC(mtp,notesLeft);
				TEC reducedTec = tec.reduceTranslators();
				double cr = reducedTec.getCompressionRatio();
				if (cr > compressionRatio) {
					bestTEC = reducedTec.copy();
					compressionRatio = cr;
				}
			}
			sb.append("\n"+bestTEC+";// [cr="+String.format("%.2f",compressionRatio)+"]");
