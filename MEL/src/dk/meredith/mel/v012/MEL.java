...
public class MEL {
	...
	void draw() {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame.setMinimumSize(new Dimension(DrawNoteSet.w,DrawNoteSet.h+23));
				frame.setResizable(false);
				frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
				PApplet embed = new DrawNoteSet(mel);
				frame.add(embed);
				embed.init();
				frame.pack();
	...
	VectorSum reflectPitch(VectorSum vectorSum) {
		VectorSum vectorSum2 = vectorSum(reflectPitch(get(vectorSum,0)));
		for(int i = 1; i < length(vectorSum); i++)
	...
	VectorSum reflectTime(VectorSum vectorSum) {
		VectorSum vectorSum2 = vectorSum(reflectTime(get(vectorSum,0)));
		for(int i = 1; i < length(vectorSum); i++)
