 ...
 * 
 * It is based on F_2STR, but it limits the allowed scale factors, s, to being in the set
 * {1,-1,2,-2,3,-3,4,-4,0.5,-0.5,0.33333,-0.33333,0.25,-0.25}.
 * 
 ...
 * - s is a scale factor for a stretch parallel to the x-axis, which
 *   must be a member of the set given above
 ...
 *
 */
public class F_2STR_FIXED extends TransformationClass {
	
	private static TreeSet<Double> ALLOWED_SCALE_FACTORS = new TreeSet<Double>(Arrays.asList(new Double[]{1.0,-1.0,2.0,-2.0,3.0,-3.0,4.0,-4.0,0.5,-0.5,1/3.0,-1/3.0,0.25,-0.25}));

	public F_2STR_FIXED() {
		super();
		setName("F_2STR_FIXED");
		setSigmaLength(4);
		setBasisSize(2);
		setPerms();
	...
	Point phi(ArrayList<Double> sigma, Point p) {
		if (!ALLOWED_SCALE_FACTORS.contains(sigma.get(0)))
			return null;
	...
	ArrayList<ArrayList<Double>> getSigmas(PointSequence objectBasis, PointSequence imageBasis) {
		...
		if (pd1 == qd1 && p1 == q1) {
			...
			if (ALLOWED_SCALE_FACTORS.contains(sigma0)) {
			...
			if (!sigma.equals(ids) && ALLOWED_SCALE_FACTORS.contains(sigma.get(0)))
				sigmas.add(sigma);
			return sigmas;
		}
		...
		ArrayList<Double> sigma = Utility.makeSigma(sigma0,sigma1,sigma2,sigma3);
		if (!sigma.equals(getIdentitySigma()) && ALLOWED_SCALE_FACTORS.contains(sigma.get(0)))
			sigmas.add(sigma);
		return sigmas;
	}
