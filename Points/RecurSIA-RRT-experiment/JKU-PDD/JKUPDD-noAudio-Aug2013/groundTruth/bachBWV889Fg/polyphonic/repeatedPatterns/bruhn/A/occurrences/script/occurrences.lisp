...
(progn
   ...
   *path&name*
   (merge-pathnames
    (make-pathname
     :directory
     '(:relative "bachBWV889Fg" "polyphonic"))
    *music-data-root*))
  (setq
   *pattname*
   (merge-pathnames
...
(loop for i from 0 to (- m 1)
  ...
  (progn
     ...
     *fname*
     (concatenate
      'string "occ" (write-to-string (+ i 1))))
...
#| The tonal answer can be defined by lowering the
pitch of the first point in P2. We will label these as
occurrences 5 and 6. |#
(setq j* '(6 7))
...
#| There is one extra partial occurrence, which can be
defined by removing the first point in P2. We will
label this as occurrence 8. |#
(setq j* 8)
(setq P* (rest P2))
(setq T* (translators-of-pattern-in-dataset P* D2))
(setq m (length T*))
...
(setq
 P3
 '((1 62) (2 60) (3 63) (4 57) (13/2 61) (7 59)
   (15/2 62) (8 60) (17/2 58) (9 56) (19/2 61)
   (10 55)))
(setq T3 (translators-of-pattern-in-dataset P3 D2))

(setq P4 (butlast P3))
(setq T4 (translators-of-pattern-in-dataset P4 D2))
