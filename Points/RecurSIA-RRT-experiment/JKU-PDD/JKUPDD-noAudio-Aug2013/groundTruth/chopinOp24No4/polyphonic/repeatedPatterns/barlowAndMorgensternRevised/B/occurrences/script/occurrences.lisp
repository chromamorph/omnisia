...
(progn
   ...
   *path&name*
   (merge-pathnames
    (make-pathname
     :directory
     '(:relative "chopinOp24No4" "polyphonic"))
    *music-data-root*))
  (setq
   *pattname*
   (merge-pathnames
...
#| Define all occurrences. This pattern consists of variations
that occur between certain ontime limits. The second occurrence
(temporally) can be considered the prototype, and so is placed
first in this list. |#
(setq *scale* 1000)
(setq
 *ontime-limits*
 (mapcar
  #'(lambda (x) (list x (+ x 23)))
  '(36 12 107.7 132 203.7 228 413.7 438)))
(setq m (length *ontime-limits*))
  ...
  (progn
    ...
    (setq
     ...
     (restrict-dataset-in-nth-to-tests
      D2 0 (list #'>= #'<) (nth i *ontime-limits*)))
    (setq Idx (indices-lookup P* D '(1 0 1 0 0)))
