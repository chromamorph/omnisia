...
(progn
   ...
   *path&name*
   (merge-pathnames
    (make-pathname
     :directory
     '(:relative "beethovenOp2No1Mvt3" "monophonic"))
    *music-data-root*))
  (setq
   *pattname*
   (merge-pathnames
    (make-pathname
     :directory
     '(:relative
       "beethovenOp2No1Mvt3" "monophonic" "repeatedPatterns"
       "barlowAndMorgenstern" "B"))
    *music-data-root*))
  (setq
   D
   (read-from-file
...
#| There are two extra partial occurrences, which can be
defined by removing the first point in P1. |#
(setq P* (rest P1))
(setq T* (translators-of-pattern-in-dataset P* D1))
(setq m (length T*))
(loop for i from 2 to (- m 1)
  ...
  (progn
    ...
    (setq
     Idx*
     (indices-lookup
      (translation P* (nth i T*)) D '(1 1 0 0 0)))
    #| Definition of ontime-pitch pairs. |#
    (setq
     Q
     (mapcar
