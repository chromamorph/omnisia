 ...
 * @date 1 July 2014
 * 
 * Computes results of fugues experiment for MML 2019 paper with Viktor Schmuck.
 * 
 ...
 *
 */
public class MML2019GAFuguesComputeResults {
...
//	private static String algorithmOutputRootFolderPathName = "D:\\Dropbox\\Work\\Research\\Papers in preparation\\2019-04-05-ISMIR-2019-OPTISIA\\2019-03-27-optisia-master\\optisia\\output\\Fugues";
	private static String algorithmOutputRootFolderPathName = "D:\\Dropbox\\Work\\Research\\Papers in preparation\\2019-06-28-MML-2019-OPTISIA\\optisia\\output\\Fugues";
...
//	private static String resultTablesFolderPathName = "D:\\Dropbox\\Work\\Research\\Data\\Giraud-fugues\\test-results";
	private static String resultTablesFolderPathName = "D:\\Dropbox\\Work\\Research\\Papers in preparation\\2019-06-28-MML-2019-OPTISIA\\optisia\\result-tables\\Fugues";
	...
	public static void main(String[] args) {
		...
		for(String algorithm : algorithmStrings) {
			...
			for(int i = 0; i < 24; i++) {
				...
				tlf1TableString.append(","+String.format("%.4f",tlf1));
				tlpTableString.append(","+String.format("%.4f",tlp));
				tlrTableString.append(","+String.format("%.4f",tlr));
			}
		...
		new File(resultTablesFolderPathName).mkdirs();
	...
	private static ArrayList<ArrayList<ArrayList<PointSet>>> getComputedOccurrenceSetsArrayForThisAlgorithm(String algorithm) {
		boolean isBoundingBox = (algorithm.contains("-bbmode")?true:false);
		boolean isSegment = (algorithm.contains("-segmode")?true:false);
		...
		for(int bwv = 846; bwv < 870; bwv++) {
			...
			if (algorithm.contains("COSIATEC")) {
				fileSuffix = "-done.cos";
			} else if (algorithm.contains("Forth")) {
				fileSuffix = "-done.Forth";
			} else if (algorithm.contains("SIATECCompress")) {
				fileSuffix = "-done.SIATECCompress";
			}
//			ArrayList<TEC> computedTECsForThisFugue = ViewCOSEncoding.readCOSIATECEncoding(outputFolderPathNameForThisAlgorithm+"/bwv"+bwv+"b-opnd"+"/bwv"+bwv+"b"+fileSuffix); 
			ArrayList<TEC> computedTECsForThisFugue = ViewCOSEncoding.readCOSIATECEncoding(outputFolderPathNameForThisAlgorithm+"/bwv"+bwv+"b"+fileSuffix); //For GA data
...
//			ArrayList<ArrayList<PointSet>> computedOccurrenceSetsForThisFugue = getOccurrenceSetsForThisTECList(computedTECsForThisFugue, isBoundingBox, isSegment, dataset);
			computedOccurrenceSetsArray.add(computedOccurrenceSetsForThisFugue);
//			System.out.println(bwv+": "+computedTECsForThisFugue.size()+" "+computedOccurrenceSetsForThisFugue.size());

		}
		return computedOccurrenceSetsArray;
	}

	public static ArrayList<ArrayList<PointSet>> getOccurrenceSetsForThisTECList(ArrayList<TEC> tecList, boolean isBoundingBox, boolean isSegment, PointSet dataset) {
		ArrayList<ArrayList<PointSet>> occurrenceSets = new ArrayList<ArrayList<PointSet>>();
		for(TEC tec : tecList) {
			ArrayList<PointSet> occurrenceSet = new ArrayList<PointSet>();
			for(Vector v : tec.getTranslators().getVectors()) {
				PointSet occurrence = tec.getPattern().translate(v);
			...
			if (tec.getPatternTECs() != null) {
				occurrenceSets.addAll(getOccurrenceSetsForThisTECList(tec.getPatternTECs(), isBoundingBox, isSegment, tec.getPattern()));
			}
		}
