	...
	public static void main(String[] args) {
		if (args.length == 0 || !(args[0].toLowerCase().endsWith(".mid") || args[0].toLowerCase().endsWith(".midi")))
			...
			try {
				...
				Path inputFilePath = new File(args[0]).toPath().toAbsolutePath();
				String inputFileName = inputFilePath.getFileName().toString();
				String outputFileName = inputFileName.substring(0, inputFileName.lastIndexOf('.'))+".gv";
